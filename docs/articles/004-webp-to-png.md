...
QuickConv の変換エンジンは Sharp 0.33（libvips ベース）を Node.js 22 で動かしています。同じ Sharp バージョンを手元で叩いて WebP → PNG の所要時間を計測した結果が以下です。中央値は 5 回試行のうちのウォームアップ 1 回を除いた median。

| 入力 WebP | 入力サイズ | 出力 PNG 変換時間（中央値） |
|---|---|---|
| 800x600（ブログ用サムネ相当） | 約 5 KB | 8ms |
| 3000x2000（高解像度写真相当） | 約 47 KB | 108ms |
