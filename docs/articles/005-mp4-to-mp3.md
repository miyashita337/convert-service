...
複数ファイルをまとめて投入することも可能です（Free プランは 1 回 3 枚まで、容量上限 10MB）。会議録画など大きなファイルを扱うなら有料プランへの切替を検討してください。

### Step 3: 出力フォーマットで MP3 を選択 → 変換

<!-- SCREENSHOT: 02-popular-conversions — フォーマット選択 UI のスクリーンショット (MP3 が選択された状態) -->
