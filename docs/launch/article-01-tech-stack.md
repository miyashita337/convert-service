...
Stripe handles all billing. The pricing tiers (from the shared constants):

| Plan | Daily limit | Max file size | Batch |
|------|-------------|---------------|-------|
| Free | 10 conversions | 10MB | 3 files |
| Pass (7-day / 30-day) | Unlimited | 20MB | 10 files |
| Plus (¥380/month) | 50 conversions | 20MB | 10 files |
| Pro (¥1,280/month) | Unlimited | 50MB | 20 files |

The checkout flow: Workers creates a Stripe Checkout Session, redirects the user, Stripe POSTs to `/api/webhook` on completion, Worker updates the D1 user record with the plan.
...
- Quality comparison preview (side-by-side before/after with compression ratio — groundwork is already in the converter)
- Format expansion: SVG → PNG, PDF → image batch
- API access for developers (Good-Better-Best pricing model already has the plumbing)
