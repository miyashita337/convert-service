...
import { findApiKeyByRawKey, incrementApiKeyUsage, getApiPlanLimit } from "../repositories/api-key-repository";
...
export function apiKeyAuthMiddleware() {
  return createMiddleware<{ Bindings: Env; Variables: AppVariables }>(
    async (c, next) => {
      ...
      const limit = getApiPlanLimit(keyInfo.plan);
        ...
        return c.json(
          {
            error: {
              ...
              message: `Monthly limit exceeded (${limit}/month). Upgrade your plan.`,
            },
          },
          429
        );
